...
def list_alpha_models(user: User = Depends(require_user), db: Session = Depends(get_db)):
    ...
    return {
        "user_models": [{"id": m.id, "name": m.name, "description": m.description,
                         ...
                         "control_factors": m.control_factors or [],
                         "return_type": m.return_type or "Total",
                         "regression_weight": m.regression_weight or "Market Cap",
                         "universe": m.universe or "Risk Model Estimation Universe",
                         "half_life": m.half_life,
                         "estimation_frequency": m.estimation_frequency or "Monthly",
                         "min_observations": m.min_observations,
